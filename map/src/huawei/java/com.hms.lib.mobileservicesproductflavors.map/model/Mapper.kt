...
package com.hms.lib.mobileservicesproductflavors.map.model
...
fun Polygon.toCommonPolygon(): MapPolygon = MapPolygon(this)
...
fun CommonLatLng.toGMSLatLng(): LatLng {
    return LatLng(lat, lng)
}

fun CommonPolygonOptions.toGMSPolygonOptions(): PolygonOptions {
    return PolygonOptions().addAll(baseLatLngs.map { it.toGMSLatLng() }).also { gmsOpts ->
