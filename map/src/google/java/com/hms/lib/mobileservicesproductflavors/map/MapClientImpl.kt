...
class MapClientImpl(context: Context, apiKey: String) : BaseMapImpl(context) {
    ...
    override fun addPolygon(commonPolygon: CommonPolygonOptions): MapPolygon {
    ...
    override fun createRouteToTheSelectedLocation(context: Context, commonLatLng: CommonLatLng):OpenNavigationResult {
        return try {
            val intentUri =
                Uri.parse("google.navigation:q=${commonLatLng.lat},${commonLatLng.lng}")
            val intent = Intent(Intent.ACTION_VIEW, intentUri)
            context.startActivity(intent)
            OpenNavigationResult()
        } catch (e: Exception) {
            OpenNavigationResult(false, e)
        }
    }
